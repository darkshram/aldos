...
sudo livecd-creator -v -d \
	...
	-c ${RUTA}/ALDOS-1.4-Xfce-x86_64.ks || exit 1
mv ALDOS64$FECHA.iso ALDOS-1.4.16-Xfce-x86_64-$FECHA.iso
sudo md5sum ALDOS-1.4.16-Xfce-x86_64-$FECHA.iso > ${RUTA}/ALDOS-1.4.16-Xfce-x86_64-$FECHA.md5sum || exit 1
sudo sha512sum ALDOS-1.4.16-Xfce-x86_64-$FECHA.iso > ${RUTA}/ALDOS-1.4.16-Xfce-x86_64-$FECHA.sha512sum || exit 1
sudo chown jbarrios:jbarrios ${RUTA}/ALDOS-1.4.16-Xfce-x86_64* || exit 1
