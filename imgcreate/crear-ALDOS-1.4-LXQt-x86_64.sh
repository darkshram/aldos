...
sudo rm -f ${RUTA}/ALDOS-1.4.16-LXQt-x86_64-$FECHA.*
sudo rm -f ${RUTA}/ALDOS-1.4.16-LXQt-x86_64.log
date > tiempos_LXQt-x86_64.txt
sudo livecd-creator -v -d \
	--logfile=${RUTA}/ALDOS-1.4.16-LXQt-x86_64.log \
	--compression-type=xz \
	--cache=/var/tmp/livecd/x86_64/CACHE/FC14/x86_64 \
	--tmpdir=/var/tmp/livecd/x86_64/ALDOS-$FECHA \
	--fslabel=ALDOS64$FECHA \
	-c ${RUTA}/ALDOS-1.4-LXQt-x86_64.ks || exit 1
mv ALDOS64$FECHA.iso ALDOS-1.4.16-LXQt-x86_64-$FECHA.iso
sudo md5sum ALDOS-1.4.16-LXQt-x86_64-$FECHA.iso > ${RUTA}/ALDOS-1.4.16-LXQt-x86_64-$FECHA.md5sum || exit 1
sudo sha512sum ALDOS-1.4.16-LXQt-x86_64-$FECHA.iso > ${RUTA}/ALDOS-1.4.16-LXQt-x86_64-$FECHA.sha512sum || exit 1
sudo chown jbarrios:jbarrios ${RUTA}/ALDOS-1.4.16-LXQt-x86_64* || exit 1
